...
let stock=document.getElementById("listaJuegos")
let venta = document.getElementById("detallePintar")
let juegosStock = []
let boton = []
let carrito = []
...
stock.addEventListener("click",e=>{
  if(e.target.classList.contains("juego-botonVenta")){
    ...
    if (verificar == -1) {
      ...
      carrito[verificar].cantidad++
      if (carrito[verificar].precioPromocional != 0) {
        carrito[verificar].subTotal = carrito[verificar].cantidad * carrito[verificar].precioPromocional
      }
      else {
        carrito[verificar].subTotal = carrito[verificar].cantidad * carrito[verificar].precio
      }
    }       
...
function pintarCarrito(juego) {
  ...
  juego.forEach(juego=>{
    if (juego.precioPromocional != 0) {
      cadena += '<div class="detalle"><div class="detalle-nombre"><p>' + juego.nombre + '</p></div><div class="detalle-precio"><div class="detalle-precio-antes"><p>$' + juego.precio + '</p></div><div class="detalle-precio-ahora"><p>$' + juego.precioPromocional + '</p></div></div><div class="detalle-cantidad"><button class="menos">-</button><p>' + juego.cantidad + '</p><button class="mas">+</button></div><div class="detalle-subTotal"><p>$' + juego.subTotal + '</p></div><div class="detalle-eliminar"><button id="'+juego.idVenta+'" class="eliminar">X</button></div></div>'
    }
    else {
      cadena += '<div class="detalle"><div class="detalle-nombre"><p>' + juego.nombre + '</p></div><div class="detalle-precio"><div class="detalle-precio-ahora"><p>$' + juego.precio + '</p></div></div><div class="detalle-cantidad"><button class="menos">-</button><p>' + juego.cantidad + '</p><button class="mas">+</button></div><div class="detalle-subTotal"><p>$' + juego.subTotal + '</p></div><div class="detalle-eliminar"><button id="'+juego.idVenta+'" class="eliminar">X</button></div></div>'
    }
  })  
  venta.innerHTML=cadena
}